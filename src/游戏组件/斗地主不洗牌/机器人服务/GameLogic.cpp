...
{
	0x01,0x02,0x03,0x13,0x23,0x33,0x04,0x14,0x24,0x09,0x19,0x0B,0x1B,	//方块 A - K
	0x22,0x12,0x28,0x07,0x08,0x27,0x17,0x18,0x0A,0x1A,0x0C,0x2B,0x1D,	//梅花 A - K
	0x21,0x11,0x05,0x15,0x25,0x26,0x16,0x06,0x29,0x2A,0x1C,0x2C,0x2D,	//红桃 A - K
	0x31,0x32,0x34,0x0D,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,	//黑桃 A - K
	0x4E,0x4F,
};
#else
const BYTE	CGameLogic::m_cbCardData[FULL_COUNT]=
...
void CGameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	...
	{
#ifdef	_DEBUG2
		cbPosition=(cbBufferCount-cbRandCount-1);
#else
		cbPosition=rand()%(cbBufferCount-cbRandCount);
#endif
		cbCardBuffer[cbRandCount++]=cbCardData[cbPosition];
...
bool CGameLogic::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, WORD wOutCardUser, WORD wMeChairID, tagOutCardResult & OutCardResult)
{
	...
	if(cbTurnCardCount==0)
	{
		...
		//////////////////////////////////////////////////////////////////////////
		//排除首出仅出3条不带牌
		if (OutCardResult.cbCardCount==3)
		{
			if (GetCardType(OutCardResult.cbResultCard, OutCardResult.cbCardCount)==CT_THREE)
			{
				BYTE cbAllSingleCardData[MAX_COUNT], cbAllSingleCount ;
				GetAllSingleCard(cbHandCardData, cbHandCardCount, cbAllSingleCardData, cbAllSingleCount,true);
				if (cbAllSingleCount>0 && GetCardLogicValue(cbAllSingleCardData[cbAllSingleCount-1])<=14)	//单牌小于A 2 王
				{
					OutCardResult.cbResultCard[OutCardResult.cbCardCount++] = cbAllSingleCardData[cbAllSingleCount-1];
				}
				else //带对
				{
					GetAllDoubleCard(cbHandCardData, cbHandCardCount, cbAllSingleCardData, cbAllSingleCount,true);
					if (cbAllSingleCount>0)
					{
						OutCardResult.cbResultCard[OutCardResult.cbCardCount++] = cbAllSingleCardData[cbAllSingleCount-1];
						OutCardResult.cbResultCard[OutCardResult.cbCardCount++] = cbAllSingleCardData[cbAllSingleCount-2];
					}
				}
...
//分析炸弹
void CGameLogic::GetAllBomCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbBomCardData[], BYTE &cbBomCardCount,bool bExact/* = false*/)
{
...
//分析顺子
void CGameLogic::GetAllLineCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbLineCardData[], BYTE &cbLineCardCount,bool bExact/* = false*/)
{
...
//分析三条
void CGameLogic::GetAllThreeCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbThreeCardData[], BYTE &cbThreeCardCount,bool bExact/* = false*/)
{
	BYTE cbTmpCardData[MAX_COUNT] ;
	CopyMemory(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
...
//分析对子
void CGameLogic::GetAllDoubleCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbDoubleCardData[], BYTE &cbDoubleCardCount,bool bExact /*= false*/)
{
	BYTE cbTmpCardData[MAX_COUNT] ;
	CopyMemory(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
...
//分析单牌
void CGameLogic::GetAllSingleCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbSingleCardData[], BYTE &cbSingleCardCount,bool bExact/* = false*/)
{
	cbSingleCardCount =0 ;
...
BYTE CGameLogic::AnalyseSinleCardCount(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbWantOutCardData[], BYTE const cbWantOutCardCount, BYTE cbSingleCardData[])
{
	...
	//函数指针
	typedef void (CGameLogic::*pGetAllCardFun)(BYTE const [], BYTE const , BYTE[], BYTE &,bool); 
	...
	for(BYTE i=0; i<24; ++i)
	{
		...
		for(BYTE j=0; j<4; ++j)
		{
			...
			//成员函数
			((*this).*pTmpGetAllCardFun)(cbTmpCardData, cbTmpCardCount, cbGetCardData, cbGetCardCount,false) ;
...
//#endif
	...
	BYTE Index=0 ;
	BYTE cbTmpHandCard[MAX_COUNT], cbTmpBombCard[MAX_COUNT], cbLargeCardCountBomb=0;	//炸弹个数	
	...
	//炸弹个数
	CopyMemory(cbTmpHandCard, m_cbAllCardData[wMeChairID], m_cbUserCardCount[wMeChairID]*sizeof(BYTE));
	GetAllBomCard(cbTmpHandCard, m_cbUserCardCount[wMeChairID], cbTmpBombCard, cbLargeCardCountBomb) ;

	
	//////////////////////////////////////////////////////////////////////////
	//叫分
	BYTE iRand = (BYTE)(GetTickCount()%3);	//随机概率
	if (cbLargeCardCount>=1 && cbSingleCardCount<=5)
	{

		if (cbLargeCardCount<=2 && cbSingleCardCount>=3 && iRand<2 && cbCurrentLandScore==255)	return 1;
		if (cbLargeCardCount>=2 && cbSingleCardCount<=4 && iRand<2 && cbCurrentLandScore<2)	return 2;
		if ((cbLargeCardCount>=3 && cbSingleCardCount<=4) 
			|| (cbLargeCardCount>=2 && cbLargeCardCountBomb>0) && cbCurrentLandScore<3)	return 3;
...
int CGameLogic::GetBombCount(const BYTE cbCardData[], BYTE cbCardCount)
{
	int nCount=0;
	tagAnalyseResult AnalyseResult ;
	AnalysebCardData(cbCardData, cbCardCount, AnalyseResult) ;

	nCount+=AnalyseResult.cbFourCount;

	for(int i=0;i<AnalyseResult.cbDoubleCount;i++)
	{
		if(GetCardLogicValue(AnalyseResult.cbDoubleCardData[i])==0x4e
		|| GetCardLogicValue(AnalyseResult.cbDoubleCardData[i])==0x4f)
			nCount++;
	}
