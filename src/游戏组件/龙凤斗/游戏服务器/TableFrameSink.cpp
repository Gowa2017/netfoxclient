...
{ 	
	...
	m_hControlInst = LoadLibrary(TEXT("LongFengDouServerControl.dll"));
	if ( m_hControlInst )
	{
		typedef void * (*CREATE)(); 
...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	_sntprintf(m_szConfigFileName,sizeof(m_szConfigFileName),TEXT("%s\\LongFengDou.ini"),szPath);
	...
	case GER_USER_LEAVE:		//用户离开
		{
			...
			if (m_wCurrentBanker!=wChairID)
			{
				...
				{
					for ( WORD wAreaIndex = AREA_LONG; wAreaIndex <= AREA_FENG; ++wAreaIndex )
					{
	...
	case IDI_GAME_END:			//结束游戏
		{
			...
			for ( WORD wUserChairID = 0; wUserChairID < GAME_PLAYER; ++wUserChairID )
			{
				...
				if ( wUserChairID == m_wCurrentBanker )
				{
					...
					if(m_lUserWinScore[wUserChairID] > pIServerUserItem->GetUserScore())
					...
					case KEY_IMMORTAL_BET:
						{
							...
							//	lBet[AREA_XIAN], lBet[AREA_PING], lBet[AREA_ZHUANG], lBet[AREA_XIAN_TIAN], lBet[AREA_ZHUANG_TIAN], lBet[AREA_TONG_DUI], lBet[AREA_XIAN_DUI], lBet[AREA_TIANGANG] );
						}
						break;
					case KEY_ROBOT_BET:
						{
	...
	for (INT_PTR nUserIdx=0; nUserIdx<m_PeizhiUserArray.GetCount(); ++nUserIdx)
	{
		...
		if (pIServerUserItem->GetGameID() == dwGameID)
		{
			for(int j = 0;j<GAME_PLAYER;j++)
			{
				...
				peizhiVec.lScore = pIServerUserItem->GetUserScore();
...
{
	//效验参数
	ASSERT((cbBetArea<=AREA_FENG)&&(lBetScore>0L));
	if ((cbBetArea>AREA_FENG)||(lBetScore<=0L)) return false;
...
bool CTableFrameSink::GetMaxPlayerScore( BYTE cbBetArea, WORD wChairID,SCORE lBetScore )
{
	...
	//已下注额
	SCORE lNowJetton = 0;
	ASSERT(AREA_COUNT<=CountArray(m_lAllBet));
	...
	//最大下注值
	SCORE lMaxBetScore = min(pIMeServerUserItem->GetUserScore(),m_lUserLimitScore);

	if(lBetCount+lBetScore>lMaxBetScore) return false;
	...
	//	//区域倍率
	//BYTE cbMultiple[AREA_MAX] = {MULTIPLE_LONG, MULTIPLE_PING, MULTIPLE_FENG};
	...
	////区域倍率
	//BYTE cbMultiple[AREA_MAX] = {MULTIPLE_ZHONG, MULTIPLE_PING, MULTIPLE_FA, 
	//								MULTIPLE_ZERO, MULTIPLE_ONE, MULTIPLE_TWO, 
	//								MULTIPLE_THREE, MULTIPLE_FOUR, MULTIPLE_FIVE, MULTIPLE_SIX,MULTIPLE_SEVEN, MULTIPLE_EIGHT, MULTIPLE_NINE, MULTIPLE_LEOPARD,MULTIPLE_TIANGANG};
	////区域输赢
...
bool CTableFrameSink::CalculateScore( OUT SCORE& lBankerWinScore, OUT tagServerGameRecord& GameRecord ,BYTE cbDispatchCount)
{
	...
	//if (cbPlayerCount == 10) GameRecord.bPlayerTwoPair = true;
	//if (cbBankerCount == 10) GameRecord.bBankerTwoPair = true;
	//if (cbPlayerCount == 11) GameRecord.bPlayerTianGang = true;
	//if (cbBankerCount == 11) GameRecord.bBankerTianGang = true;
	for (int i=0;i<3;i++)
	{
		if (cbWinArea[i] == 1)
		{
			GameRecord.cbGameResult = i;
		}
	}

	//GameRecord.bPlayerTwoPair = cbWinArea[AREA_XIAN_DUI] == TRUE;
	//GameRecord.bBankerTwoPair = cbWinArea[AREA_TIANGANG] == TRUE;
	...
	for (WORD wChairID = 0; wChairID < GAME_PLAYER; wChairID++)
	{
		...
		for ( WORD wAreaIndex = 0; wAreaIndex < AREA_MAX; ++wAreaIndex )
		{
			...
			else if ( cbWinArea[AREA_PING] == TRUE && ( wAreaIndex == AREA_LONG || wAreaIndex == AREA_FENG ) )
			{	
				lUserLostScore[wChairID] -= m_lPlayBet[wChairID][wAreaIndex]/2;
				m_lPlayScore[wChairID][wAreaIndex] -= m_lPlayBet[wChairID][wAreaIndex]/2;
				lBankerWinScore += m_lPlayBet[wChairID][wAreaIndex]/2;

				//系统得分
				if(bIsAndroidUser)
					lSystemScore -= m_lPlayBet[wChairID][wAreaIndex]/2;
				if (m_wCurrentBanker == INVALID_CHAIR || bIsBankerAndroidUser)
					lSystemScore += m_lPlayBet[wChairID][wAreaIndex]/2;

				//庄家得分
				if ( m_wCurrentBanker != INVALID_CHAIR && m_wCurrentBanker != wChairID )
					m_lPlayScore[m_wCurrentBanker][wAreaIndex] += m_lPlayBet[wChairID][wAreaIndex]/2;
		...
		if ( m_lUserWinScore[wChairID] > 0 )
		{
			...
			m_lUserRevenue[wChairID] = (SCORE) ((SCORE((( m_lUserWinScore[wChairID] ))*fRevenuePer/1000*100))/100.0);
...
// 			m_lUserRevenue[m_wCurrentBanker]  = LONGLONG(m_lUserWinScore[m_wCurrentBanker]*fRevenuePer);
			SCORE fRevenuePer=m_pGameServiceOption->wRevenueRatio;
			m_lUserRevenue[m_wCurrentBanker] = (SCORE) ((SCORE((( m_lUserWinScore[m_wCurrentBanker] ))*fRevenuePer/1000*100))/100.0);
...
void CTableFrameSink::DeduceWinner(BYTE* pWinArea)
{
	...
	int nWinShangMen=m_GameLogic.CompareCard(m_cbTableCardArray[0],1,m_cbTableCardArray[1],1);

	if(nWinShangMen == -1)
		pWinArea[0] = true;
	else if(nWinShangMen == 1)
		pWinArea[2] = true;
	else if(nWinShangMen == 0)
		pWinArea[1] = true;
