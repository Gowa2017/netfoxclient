...
bool __cdecl CServerControlItemSink::ServerControl(WORD wSubCmdID, const void * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem, ITableFrame * pITableFrame,const tagGameServiceOption * pGameServiceOption)
{
	...
	case RQ_SET_WIN_AREA:
		{	
			...
			if(m_nControlCount>0 && m_nControlCount<4)
			{
...
{
	if (m_nControlCount>0 && m_nControlCount<4 && m_cbExcuteTimes > 0)
	{
		return true;
	}
	return false;
...
bool __cdecl CServerControlItemSink::ControlResult(BYTE	cbTableCardArray[], BYTE cbCardCount[])
{
	...
	if (m_nControlCount == 2)
	{
		BYTE cbControlArea[2] = {-1,-1};
		BYTE index = 0;
		for (int i=0; i<AREA_MAX;i++)
		{
			if (m_bWinArea[i] == 1)
			{
				cbControlArea[index] = i;
				index++;
				if (index == 2) break;					
			}
		}
		...
		do 
		{
			...
			if (cbWinArea[cbControlArea[0]] == true && cbWinArea[cbControlArea[1]] == true)	bFindCard = true;
	...
	if (m_nControlCount == 3)
	{
		BYTE cbControlArea[3] = {-1,-1,-1};
		BYTE index = 0;
		for (int i=0; i<AREA_MAX;i++)
		{
			if (m_bWinArea[i] == 1)
		...
		do 
		{
			...
			if (cbWinArea[cbControlArea[0]] == true && cbWinArea[cbControlArea[1]] == true && cbWinArea[cbControlArea[2]] == true)	bFindCard = true;
