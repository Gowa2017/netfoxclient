...
{	
	...
	for(int i = 0;i<3;i++)
	{
		if(i != 1)
		{
			for(int j = 3;j<AREA_MAX;j++)
			{
				...
				{
					//当开这个区域需要赔付金币
					SCORE lBKScore = m_lAllBet[i] * cbMultiple[i] +   (m_lAllBet[cbBetArea]) *  cbMultiple[cbBetArea] + m_lAllBet[j]*  cbMultiple[j];
					//这是庄家赚的金币
					...
					if(lBankerScore + lBankerRemain - lBKScore < lBet  )
					{
						lBet = lBankerScore + lBankerRemain - lBKScore;
					}
				}
			}
		}
		...
		{
			//当开这个区域需要赔付金币
			SCORE lBKScore = m_lAllBet[i] * cbMultiple[i] +   (m_lAllBet[cbBetArea]) *  cbMultiple[cbBetArea] ;
			//这是庄家赚的金币
	...
	return lBet/(cbMultiple[cbBetArea] - 1);
...
void CAndroidUserItemSink::BankOperate(BYTE cbType)
{
...

	{
		...
		{			
			...
			if (lSaveScore > 0) 
			{
				lSaveScore += (rand()%((LONGLONG)((m_pIAndroidUserItem->GetAndroidParameter()->lTakeMaxScore - m_pIAndroidUserItem->GetAndroidParameter()->lTakeMinScore)*100)/100));
				m_pIAndroidUserItem->PerformSaveScore(lSaveScore);
			}
		...
		else if (lRobotScore < m_pIAndroidUserItem->GetAndroidParameter()->lTakeMinScore)
		{
			...
			if (lScore > 0)
			{
				lScore += (rand()%((LONGLONG)((m_pIAndroidUserItem->GetAndroidParameter()->lTakeMaxScore - m_pIAndroidUserItem->GetAndroidParameter()->lTakeMinScore)*100)/100));
				m_pIAndroidUserItem->PerformTakeScore(lScore);
			}
