...
void __cdecl CClientControlItemSinkDlg::UpdateUserBet(bool bReSet)
{
	...
	for(INT i=0; i<m_UserBetArray.GetCount(); i++)
	{
		...
		strInfo.Format(TEXT("%.2lf"), m_UserBetArray[i].lUserStartScore);
		m_listUserBet.SetItemText(nItemIndex, 4, strInfo);

		strInfo.Format(TEXT("%.2lf"), m_UserBetArray[i].lUserWinLost);
		...
		for(BYTE j=0; j<AREA_MAX; j++)
		{
			...
			strInfo.Format(TEXT("%.2lf"), m_UserBetArray[i].lUserBet[j]);
	...
	for(BYTE k=0; k<AREA_MAX; k++)
	{
		...
		CString strInfoAll;
		strInfoAll.Format(TEXT("%.2lf"), lAllBetScore[k]);
...
void CClientControlItemSinkDlg::OnBnClickedBtnUpdateStorage()
{
	...
	{
		CString str;
		str.Format(TEXT("[当前库存值]输入范围为0-%.2lf"),LLONG_MAX);
		AfxMessageBox(str);
			return;
	}
	...
	{
		CString str;
		str.Format(TEXT("[库存上限1]输入范围为0-%.2lf"),LLONG_MAX);
		AfxMessageBox(str);
		return;
	}
	...
	{
		CString str;
		str.Format(TEXT("[库存上限2]输入范围为0-%.2lf"),LLONG_MAX);
		AfxMessageBox(str);
		return;
	}
