...
m_lCellScore = 0;
  ZeroMemory(m_iMaxTimes,sizeof(m_iMaxTimes));
...
bool CAndroidUserItemSink::OnEventTimer(UINT nTimerID)
{
  try
  {
	...
	case IDI_UP:
		{
			printf("机器人退出\n");
			//if(m_pIAndroidUserItem->GetMeUserItem()->GetUserInfo()->cbUserStatus < US_READY)
			//m_pIAndroidUserItem->SendSocketData(SUB_S_ANDROIDQIANGZHIUP);
			return true;
		}
      case IDI_DELAY_TIME:
      {
      ...
      case IDI_START_GAME:    //开始定时器
      {
		...
		//m_pIAndroidUserItem->KillGameTimer(IDI_UP);
        //发送准备
      ...
      case IDI_CALL_BANKER:   //叫庄定时
      {
        ...
        //CallBanker.bBanker = rand()%5+1;

		int usercount = 0;
		for(int j = 0;j<GAME_PLAYER;j++)
		{
			if(m_cbPlayerStatus[j] == true)usercount++;
		}
		...
		for(int i =0;i<4;i++)
		{
			if(m_pIAndroidUserItem->GetMeUserItem()->GetUserInfo()->lScore >= ((usercount-1)*4*m_lCellScore*(i+1)))
			{
				cbCall[nIndex] = i + 1;
				nIndex++;
			}
		...
		cbCall[nIndex] = 5;
		nIndex++;
		CallBanker.bBanker = cbCall[rand()%nIndex];
        //发送信息
      ...
      case IDI_USER_ADD_SCORE:  //加注定时
      {
		...
		IServerUserItem *pUserItem = m_pIAndroidUserItem->GetMeUserItem();
		SCORE  lMeScore = pUserItem->GetUserScore();
		for(int i = 0;i<4;i++)
		{
			if( (m_wBankerUser != INVALID_CHAIR && lMeScore >= 4*m_lCellScore*(i+1)*m_iMaxTimes[m_wBankerUser])
				&& (m_lTurnMaxScore >= (usercount - 1)*4*m_lCellScore*(i+1)*m_iMaxTimes[m_wBankerUser]))
			{
				cbCall[nIndex] = i + 1;
				nIndex++;
			}
		...
		if(nIndex > 0)
			AddScore.lScore = cbCall[rand()%nIndex] * m_lCellScore;
		else
			AddScore.lScore = m_lCellScore;
		if(usercount == 0 || m_wBankerUser == INVALID_CHAIR )
			        AddScore.lScore=m_lCellScore;
...
bool CAndroidUserItemSink::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, void * pData, WORD wDataSize)
{
  switch(cbGameStatus)
  {
    case GAME_STATUS_FREE:    //空闲状态
    {
      ...
      UINT nElapse= TIME_LESS + (rand() % TIME_USER_START_GAME);
      m_pIAndroidUserItem->SetGameTimer(IDI_START_GAME,nElapse);
	m_pIAndroidUserItem->SetGameTimer(IDI_UP,15);
      return true;
    }
    case GS_TK_CALL:  // 叫庄状态
    {
      ...
      //始叫用户
      if(pStatusCall->wCallBanker==m_pIAndroidUserItem->GetChairID()&& pStatusCall->cbDynamicJoin == false)
      {
        //叫庄时间
    ...
    case GS_TK_SCORE: //下注状态
    {
      ...
      //设置筹码
	  if(pStatusScore->lTurnMaxScore>0L && pStatusScore->lTableScore[wMeChairId]==0L && pStatusScore->cbDynamicJoin == false)
      {
        //下注时间
    ...
    case GS_TK_PLAYING: //游戏状态
    {
      ...
      //控件处理
      if(pStatusPlay->bOxCard[wMeChiarID]==0xff&& pStatusPlay->cbDynamicJoin == false)
      {
        //开牌时间
...
bool CAndroidUserItemSink::OnSubCallBanker(const void * pBuffer, WORD wDataSize)
{
  ...
  CopyMemory(m_cbPlayerStatus,pCallBanker->cbPlayerStatus,sizeof(m_cbPlayerStatus));

  //始叫用户
  //if(pCallBanker->wCallBanker==m_pIAndroidUserItem->GetChairID())
  {
    //叫庄时间
    UINT nElapse = TIME_LESS + (rand() % TIME_USER_CALL_BANKER) + 5;
    m_pIAndroidUserItem->SetGameTimer(IDI_CALL_BANKER,nElapse) ;
  }
...
bool CAndroidUserItemSink::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
  ...
  CopyMemory(m_iMaxTimes,pGameStart->iMaxTimes,sizeof(m_iMaxTimes));
  m_wBankerUser = pGameStart->wBankerUser;
  CopyMemory(m_cbPlayerStatus,pGameStart->cbPlayerStatus,sizeof(m_cbPlayerStatus));
  //设置变量
  m_lTurnMaxScore=pGameStart->lTurnMaxScore;

  //设置筹码
  if(m_wBankerUser != m_pIAndroidUserItem->GetMeUserItem()->GetChairID())
  {
    //下注时间
    UINT nElapse = TIME_LESS + (rand() % TIME_USER_ADD_SCORE)+ 2;
    m_pIAndroidUserItem->SetGameTimer(IDI_USER_ADD_SCORE,nElapse) ;
  }
...
bool CAndroidUserItemSink::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
  ...
  {
    UINT nElapse= TIME_LESS + (rand() % TIME_USER_START_GAME) + 4;
    m_pIAndroidUserItem->SetGameTimer(IDI_START_GAME,nElapse);
	m_pIAndroidUserItem->SetGameTimer(IDI_UP,15);
    //BankOperate(3);
  }
