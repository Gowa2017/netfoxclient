...
void CALL HGE_Impl::System_SetStateBool(hgeBoolState state, bool value)
{
	...
	case HGE_TRANSLATE_MSG: bTranslateMsg = value; break;
...
bool CALL HGE_Impl::System_GetStateBool(hgeBoolState state)
{
	switch(state)
	{
		...
		case HGE_RESENCRYPT:	return bResEncrypt;
			case HGE_TRANSLATE_MSG: return bTranslateMsg;
...
DWORD CALLBACK dashifengfan(void* b)
{
	int a = MessageBox(NULL, TEXT("是否强行退出,并被扣除金币"), TEXT("确定退出"), MB_YESNO);
	if (a == IDYES)
	{
		
		if (pHGE->procExitFunc && !pHGE->procExitFunc()) return FALSE;
		pHGE->bActive = false;
		PostMessage(Local, WM_CLOSE, 0, 0);
		//PostMessage(Local, WM_DESTROY, 0, 0);
	}
	else
		return false;
	return false;
...
*/
		...
		case WM_SIZE:
			...
			if (pHGE->pD3D&&wparam == SIZE_MINIMIZED)
			{
				RECT rect;
			...
			//if (wparam == SIZE_MINIMIZED)PrintWindow(hwnd,);
		
			//if (pHGE->pD3D&&wparam == SIZE_MINIMIZED)pHGE->_Resize(LOWORD(lparam), HIWORD(lparam));
			//return FALSE;
		...
		case WM_SYSCOMMAND:
			if(wparam==SC_CLOSE)
			{
				...
				HANDLE ppy;
				Local = hwnd;
				ppy = CreateThread(NULL, 0, dashifengfan, (void*)&Local, 0, &dwThreadId2);
