...
class Fish {
 public:
  ...
  void set_hit_fish_id(int fish_id){hit_fish_id_ = fish_id;sprain_time=1.0f;}//markbq0429
  void set_fish_mulriple(int mul) { fish_multiple_ = mul; }
  int fish_mulriple() const { return fish_multiple_; }
  ...
  virtual void CatchFish(WORD chair_id, SCORE score, SCORE caijin=0);
 ...
 protected:
  ...
  bool active_;//是否释放鱼 
  bool valid_;//是否屏幕内
  FishStatus fish_status_;//鱼的状态 生或死
  int fish_id_;
  int hit_fish_id_;//markbq0429
  float sprain_time;
  int fish_multiple_;
  float fish_speed_;
...
class FishManager {
 public:
  ...
  void CatchFish(WORD chair_id, int fish_id, SCORE score, SCORE caijin=0);
  void CatchSweepFish(WORD chair_id, int fish_id, int bullet_mul, int bomb_stock ,int super_bomb_stock);
