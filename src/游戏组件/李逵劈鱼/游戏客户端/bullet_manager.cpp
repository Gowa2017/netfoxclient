...
Bullet::Bullet(BulletKind bullet_kind, int bullet_id, int bullet_mulriple, WORD firer_chair_id, float net_radius)
  ...
  angle_(0.f), trace_index_(0), fade_(false) {
...
bool Bullet::OnFrame(float delta_time) {
  ...
  if (bullet_status_ == BULLET_FLYING) {
    ...
    if (!ani_net_->IsPlaying()) {
      if (fade_) {
        DWORD color = ani_net_->GetColor();
        int alpha = GETA(color);
        //alpha -= 10;//bq0418
		alpha -= 18;//bq0418
        if (alpha <= 5) {
          color = 0x00FFFFFF;
          ani_net_->SetColor(color);
          return true;
        } else {
...
int BulletManager::Fire(float src_x_pos, float src_y_pos, float angle, BulletKind bullet_kind,
  ...
  //WCHAR  ss1[32]={'a'};
  //id46++;
  //wsprintfW(ss1, L"%d", id46);
  //WritePrivateProfileString(_T("Test"),_T("id46"),ss1,_T("D:\\log.ini"));
  ////bq0415
...
void BulletManager::GetTargetPoint(WORD firer_chair_id, float src_x_pos, float src_y_pos, float angle, float& target_x_pos, float& target_y_pos) {
  ...
  } else if (firer_chair_id == 5|| firer_chair_id == 6 || firer_chair_id == 4) {
	  if (angle<3.5f)
		  angle += 6.28;
    if (angle == M_PI / 2) {
      target_x_pos = screen_width;
