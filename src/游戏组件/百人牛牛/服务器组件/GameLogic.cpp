...
//混乱扑克
void CGameLogic::RandCardListLose(BYTE cbCardBuffer[], BYTE cbBufferCount,BYTE cbAllCardData[54],BYTE cbCardCount)
{
	//混乱准备
	BYTE cbCardData[54];
	...
	do
	{
		cbPosition=rand()%(CountArray(cbCardData)-cbRandCount - (54-cbCardCount));
		cbCardBuffer[cbRandCount++]=cbCardData[cbPosition];
		if (cbCardData[cbPosition] > 100)
		{
			int nn = 0;
		}
		cbCardData[cbPosition]=cbCardData[CountArray(cbCardData)-cbRandCount];
...
int CGameLogic::CompareCard(const BYTE cbFirstCardData[], BYTE cbFirstCardCount,const BYTE cbNextCardData[], BYTE cbNextCardCount,BYTE &Multiple)
{
  ...
  if(cbFirstCardType != cbNextCardType)
  {
    ...
    {
      if(cbNextCardType>=CT_SPECIAL_NIUYING)
      {
		 Multiple = 5;
      }
      ...
      else if(cbNextCardType == 11)
      {
			Multiple = 3;
      }
	  else if(cbNextCardType >= 9)
	  {
		  Multiple = 2;
	  }
	  else
    ...
    {
		if(cbFirstCardType>=CT_SPECIAL_NIUYING)
		{
			Multiple = 5;
		}
		...
		else if(cbFirstCardType == 11)
		{
			Multiple = 3;
		}
		else if(cbFirstCardType >= 9)
		{
			Multiple = 2;
		}
		else
...
bool CGameLogic::RemoveGoodCardData(BYTE cbGoodcardData[5], BYTE cbGoodCardCount, BYTE cbCardData[54], BYTE cbCardCount) 
{
	//检验数据
	...
	//定义变量
	BYTE cbDeleteCount=0,cbTempCardData[54];
