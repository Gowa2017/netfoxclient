...
m_WinnerScore = 0;
  ...
  m_hInst = LoadLibrary(TEXT("OxTobiNewServerControl.dll"));
  if(m_hInst)
  {
    typedef void * (*CREATE)();
  ...
  if(m_bFirstInit)
  {
    CString strName = GetFileDialogPath()+  "\\OxTobiNewBattle.log";
...
bool CTableFrameSink::OnEventGameStart()
{
	...
	for(WORD i = 0; i < GAME_PLAYER; i++)
	{
		...
		m_lTableScore[i] = m_pGameServiceOption->lCellScore;
	}
	...
	for(WORD i=0; i<m_wPlayerCount; i++)
	{
		...
		CopyMemory(AllCard.cbCardData[i],&bTempArray[wIndex*MAX_COUNT],MAX_COUNT);
		wIndex++;
	}
  //闲家全到
	...
	//}
	SendCard.wCallBanker = m_wBankerUser;
	SendCard.lCellScore = m_pGameServiceOption->lCellScore;
	CopyMemory(SendCard.cbPlayStatus, m_cbPlayStatus, sizeof(m_cbPlayStatus));
	...
	for (WORD i=0;i<m_wPlayerCount;i++)
	{
		...
		IServerUserItem *pIServerUser=m_pITableFrame->GetTableUserItem(i);
		if(pIServerUser==NULL)continue;
		CopyMemory(AllCard.cbSurplusCardData,&bTempArray[wIndex*MAX_COUNT],52-wIndex*MAX_COUNT);
  ...
  switch(cbReason)
  {
    ...
    case GER_NORMAL:    //常规结束
    {
		...
		for(WORD i = 0; i < GAME_PLAYER; i++)
		{
			...
			if((m_GameLogic.GetCardType(m_cbHandCardData[wWinner], MAX_COUNT) == OX_FIVEKING ? true : false))
			{
...
bool CTableFrameSink::OnUserCallBanker(WORD wChairID, BYTE bBanker)
{
  ...
  m_nUserCallStatus[wChairID] = bBanker;
  //叫庄人数
...
void CTableFrameSink::AndroidHuanPai()
{
	...
	if(bIsWin)
	{
		for(int i =0;i<GAME_PLAYER;i++)
		{
			...
			if(pServerUserItem->IsAndroidUser())
			{
				...
				ProbeJudge(i,CHEAT_TYPE_WIN);  //指定玩家胜利
				//BeginChange(true);
				break; 
			}
	...
	else
	{
		for(int i =0;i<GAME_PLAYER;i++)
		{
			...
			if(!pServerUserItem->IsAndroidUser())
			{
				ProbeJudge(i,CHEAT_TYPE_WIN);  //指定玩家失败
				//BeginChange(true);
				break; 
			}
...
void CTableFrameSink::BeginChange(bool IsLoss )
{
	...
	{
		if(j == m_wBankerUser || m_cbPlayStatus[j]!=true)
			continue;
		//交换手牌
		BYTE bTempCardData[5] = {};
		memcpy(bTempCardData,m_cbHandCardData[m_wBankerUser],sizeof(bTempCardData));
		memcpy(m_cbHandCardData[m_wBankerUser],m_cbHandCardData[j],sizeof(bTempCardData));
		memcpy(m_cbHandCardData[j],	bTempCardData,sizeof(bTempCardData));
