...
function GameFrameEngine:onSocketConfigEvent(sub, dataBuffer)
	...
	if sub == yl.SUB_GR_CONFIG_SERVER then
		...
		print(
			"房间配置[table:" ..
				self._wTableCount ..
					"][chair:" .. self._wChairCount .. "][type:" .. self._wServerType .. "][rule:" .. self._dwServerRule .. "]"
		)
...
function GameFrameEngine:onSocketFrameEvent(sub, dataBuffer)
	...
	elseif sub == game_cmd.SUB_GF_USER_VOICE then
		AudioRecorder:getInstance():saveRecordFile(
			dataBuffer,
			function(uid, tid, spath)
				local msgTab = {}
				msgTab.uid = uid
				msgTab.tid = tid
				msgTab.spath = spath
...
function GameFrameEngine:popVocieMsg()
	...
	if type(msgTab) == "table" then
		...
		if nil ~= useritem then
			...
			AudioRecorder:getInstance():setFinishCallBack(
				self._nPlayVoiceId,
				function(voiceid, filename)
