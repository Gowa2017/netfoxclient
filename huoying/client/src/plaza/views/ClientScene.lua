...
function ClientScene:onCreate()
	...
	-- 背景
	self._bg = ccui.ImageView:create("background_2.jpg"):move(display.center):addTo(self)
...
function ClientScene:unregisterNotify()
	...
	NotifyMgr:getInstance():unregisterNotify(
		chat_cmd.MDM_GC_USER,
		chat_cmd.SUB_GC_RESPOND_NOTIFY,
		"client_friend_response"
	)
	NotifyMgr:getInstance():unregisterNotify(
		chat_cmd.MDM_GC_USER,
		chat_cmd.SUB_GC_INVITE_GAME_NOTIFY,
		"client_friend_invite"
	)
	NotifyMgr:getInstance():unregisterNotify(
		chat_cmd.MDM_GC_USER,
		chat_cmd.SUB_GC_USER_SHARE_NOTIFY,
		"client_friend_share"
	)
	NotifyMgr:getInstance():unregisterNotify(
		chat_cmd.MDM_GC_USER,
		chat_cmd.SUB_GC_INVITE_PERSONAL_NOTIFY,
		"client_pri_friend_invite"
	)
...
function ClientScene:onChangeShowMode(nTag, param)
	...
	--控制宝箱、喇叭显示
	local infoShow = (tag == yl.SCENE_GAMELIST or bRoomList)
	 --or tag == yl.SCENE_ROOM)
			...
			if nil == self.m_spGameTrumpetBg then
				...
				if nil ~= frame then
					...
					if nil ~= runScene and nil ~= trumpetBg then
						...
						local notifyText =
							cc.Label:createWithTTF("", "fonts/round_body.ttf", 24):addTo(notifyClip):setTextColor(cc.c4b(255, 191, 123, 255)):setAnchorPoint(
							cc.p(0, 0.5)
						):enableOutline(cc.c4b(79, 48, 35, 255), 1):move(700, 0)
						...
						function()
							if 0 ~= #self.m_gameTrumpetList then
								...
								self.m_spGameTrumpetBg:runAction(
									cc.Sequence:create(
										cc.ScaleTo:create(0.5, 0.0001, 1.0),
										cc.CallFunc:create(
											function()
												self.m_spGameTrumpetBg:removeFromParent()
												self.m_spGameTrumpetBg = nil
...
function ClientScene:createPasswordEdit(placeholder, confirmFun)
	...
	local bg = display.newSprite("plaza/plaza_query_bg.png"):move(appdf.WIDTH / 2, appdf.HEIGHT / 2):addTo(layout)
	local bg_size = bg:getContentSize()

	--编辑框
	local editpass =
		ccui.EditBox:create(cc.size(490, 67), "RoomList/sp_pwedit_bg.png"):move(bg_size.width * 0.5, 180):setAnchorPoint(
		cc.p(0.5, 0.5)
	):setFontName("fonts/round_body.ttf"):setPlaceholderFontName("fonts/round_body.ttf"):setFontSize(24):setPlaceholderFontSize(
		24
	):setMaxLength(32):setInputFlag(cc.EDITBOX_INPUT_FLAG_PASSWORD):setInputMode(cc.EDITBOX_INPUT_MODE_SINGLELINE):setPlaceHolder(
		placeholder
	):setFontColor(cc.c4b(254, 164, 107, 255)):addTo(bg)
	...
	ccui.Button:create("p_bt_close_0.png", "p_bt_close_1.png", "p_bt_close_0.png", UI_TEX_TYPE_PLIST):move(
		bg_size.width - 50,
		bg_size.height - 50
	):setTag(BT_CLOSE_DLG):addTo(bg):addTouchEventListener(btnEvent)

	ccui.Button:create("General/bt_confirm_0.png", "General/bt_confirm_1.png"):move(bg_size.width * 0.5, 50):setTag(
		BT_CONFIRM
	):addTo(bg):addTouchEventListener(btnEvent)
end
			...
			if type(sjstable) == "table" then
				...
				if type(data) == "table" then
					...
					if true == valid then
						...
						if
							score ~= GlobalUserItem.lUserScore or bean ~= GlobalUserItem.dUserBeans or ingot ~= GlobalUserItem.lUserIngot or
...
function ClientScene:createHelpBtn(pos, zorder, url, parent)
	...
	local btn =
		ccui.Button:create("pub_btn_introduce_0.png", "pub_btn_introduce_1.png", "pub_btn_introduce_0.png", UI_TEX_TYPE_PLIST)
	btn:setPosition(pos)
...
function ClientScene:createVoiceBtn(pos, zorder, parent)
	...
	pos = pos or cc.p(100, 100)
	local btn =
		ccui.Button:create("btn_voice_chat_0.png", "btn_voice_chat_1.png", "btn_voice_chat_0.png", UI_TEX_TYPE_PLIST)
	btn:setPosition(pos)
