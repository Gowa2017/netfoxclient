...
//获取金币
SCORE CGoldControl::GetGold()
{
  SCORE lGold=0L;
  for(SCORE i=0; i<CountArray(m_lGoldCell); i++)
  {
    lGold+=m_lGoldCell[i]*(SCORE)pow(10L,(int)i);
  }
  return lGold;
}

//设置金币
void CGoldControl::SetGold(SCORE lGold)
{
  //调整参数
  if(lGold>m_lMaxGold)
  {
...
//设置用户最高下注数
void CGoldControl::SetMaxGold(SCORE lMaxGold)
{
  //效验改变
...
//绘画金币
void CGoldControl::DrawGoldCell(CDC * pDC, int nXPos, int nYPox, SCORE lGold)
{
  if(lGold<10L)
  {
    TCHAR szBuffer[12];
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
  ...
  {
    SCORE lAddGold=(SCORE)pow(10L,iCellPos);
...
void CGoldControl::OnRButtonUp(UINT nFlags, CPoint point)
{
  ...
  {
    SCORE lAddGold=9L*(SCORE)pow(10L,iCellPos);
    if((GetGold()+lAddGold)>m_lMaxGold)
    {
      return;
    }
