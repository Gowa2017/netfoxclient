...
// 唯一的一个 CCfishbaojingApp 对象

CCfishbaojingApp theApp;


// CCfishbaojingApp 初始化
...
bool CCfishbaojingApp::PipeLoading()
{
	...
	{
		OutputDebugString(L"初始化失败 Error1！ fish_config150");
		return false;
	}
	//打开 命名消息管道
	...
	{
		OutputDebugString(L"初始化失败 Error2 fish_config！160");
		m_hServerPipe = NULL;
		return false;
	}
	...
	{
		OutputDebugString(L"写入数据失败！ fish_config170");
		PostQuitMessage(-1);
		return false;
	}
	...
	{
		OutputDebugString(L"创建命名管道失败！ fish_config180");
		m_hClientPipe = NULL;
		return false;
	}
	HANDLE hEvent;
	...
	if (!hEvent)
	{
		OutputDebugString(L"创建事件对象失败！fish_config190");
		CloseHandle(m_hClientPipe);
		m_hClientPipe = NULL;
		return false;
	}
	...
	if (!ConnectNamedPipe(m_hClientPipe, &ovlap)) //等待客户端连接命名管道
	{
		...
		{
			OutputDebugString(L"等待客户端连接失败！fish_config200");
			CloseHandle(m_hClientPipe);
			CloseHandle(hEvent);
			m_hClientPipe = NULL;
			return false;
