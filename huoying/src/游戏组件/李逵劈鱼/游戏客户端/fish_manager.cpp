...
Fish::Fish(FishKind fish_kind, int fish_id, int fish_multiple, float fish_speed, float bounding_box_width, float bounding_box_height, float hit_radius)
  ...
  stop_index_(0), stop_count_(0), current_stop_count_(0),hit_fish_id_(0),sprain_time(1.0f) {
...
bool Fish::OnFrame(float delta_time, bool lock) {
  ...
  if (fish_id_==hit_fish_id_&&sprain_time>0)
  {
	  //ani_fish_->SetBlendMode(BLEND_COLORMUL);
	  ani_fish_->SetColor(ARGB(255,255,0,0));
	  sprain_time-=delta_time*4;
  ...
  else
  {
	 // ani_fish_->SetBlendMode(BLEND_COLORMUL);
	  sprain_time=0;
	  ani_fish_->SetColor(ARGB(255,255,255,255));
  }
  CheckValid();
...
void Fish::CatchFish(WORD chair_id, SCORE score, SCORE caijin) {
  ...
  } else if (fish_kind_ == FISH_KIND_23 || fish_kind_ == FISH_KIND_24 || caijin >0 ) {
...
bool FishLK::OnRender(float offset_x, float offset_y, float hscale, float vscale) {
  ...
  if (fish_status_ == FISH_DIED) {
    ...
    ani_score_->OnRender(offset_x, offset_y, hscale, vscale);
	  ...
	  ////markbq0429
	  //ani_fish_->SetColor(ARGB(0,255,0,0));
	  ////
    ani_fish_->RenderEx(offset_x + fish_trace.x, offset_y + fish_trace.y, fish_trace.angle, hscale, vscale);
...
void FishSanSi::CatchFish(WORD chair_id, SCORE score, SCORE caijin) {
  Fish::CatchFish(chair_id, score, caijin);
...
bool FishManager::LoadGameResource() {
  ...
  //ani_ding_ = resource_manager->GetAnimation("Ding");
...
bool FishManager::OnRender(float offset_x, float offset_y, float hscale, float vscale) {
  ...
  //if (ani_ding_->IsPlaying()) {
    ...
    //ani_ding_->Render(screen_width / 2, screen_height / 2);
  }
...
Fish* FishManager::ActiveFish(FishKind fish_kind, int fish_id, int fish_multiple, float fish_speed,
  ...
  if (fish_kind == FISH_KIND_21) {
    fish = new FishLK(fish_id, fish_multiple, fish_speed*0.9, bounding_box_width, bounding_box_height, hit_radius);
...
bool FishManager::NetHitTest(Bullet* bullet) {
  ...
  for (iter = fish_vector_.begin(); iter != fish_vector_.end(); ++iter) {
    ...
    if (fish->NetHitTest(bullet)) {
		if (bullet->firer_chair_id() == me_chair_id)
		{
			fish->set_hit_fish_id(fish->fish_id());//markbq0429
		}
...
void FishManager::CatchFish(WORD chair_id, int fish_id, SCORE score, SCORE caijin) {
  assert(chair_id < GAME_PLAYER);
  Fish* fish = GetFish(fish_id);
  if (fish == NULL) {
	    //::MessageBox( NULL, _T("not found fish"), NULL, MB_OK );
...
void FishManager::CatchSweepFish(WORD chair_id, int fish_id, int bullet_mul, int bomb_stock ,int super_bomb_stock) {
  Fish* fish = GetFish(fish_id);
  if (fish == NULL) {
    //assert(!"CatchFish: not found fish");
	 //::MessageBox( NULL, _T("not found fish"), NULL, MB_OK );
  ...
  if (fish->fish_kind() == FISH_KIND_23) {
    ...
    for (iter = fish_vector_.begin(); iter != fish_vector_.end(); ++iter) {
      ...
      if (InsideScreen(fish_pos) && box.ComputeCollision(fish_pos.x, fish_pos.y)) {
        ...
        total_score += fish_temp->fish_mulriple() * bullet_mul;
        if (total_score >= bomb_stock)
          break;
    ...
    for (iter = fish_vector_.begin(); iter != fish_vector_.end(); ++iter) {
      ...
      if (InsideScreen(fish_pos)) {
        catch_sweep_fish.catch_fish_id[catch_sweep_fish.catch_fish_count++] = fish_temp->fish_id();
        total_score += fish_temp->fish_mulriple() * bullet_mul;
        if (total_score >= super_bomb_stock)
          break;
        if (catch_sweep_fish.catch_fish_count >= arraysize(catch_sweep_fish.catch_fish_id)) break;
      }
    }
    ...
    for (iter = fish_vector_.begin(); iter != fish_vector_.end(); ++iter) {
      ...
      if (InsideScreen(fish_pos)) {
        catch_sweep_fish.catch_fish_id[catch_sweep_fish.catch_fish_count++] = fish_temp->fish_id();
        if (catch_sweep_fish.catch_fish_count >= kFishKindCount[fish->fish_kind() - FISH_KIND_31])
          break;
        if (catch_sweep_fish.catch_fish_count >= arraysize(catch_sweep_fish.catch_fish_id)) break;
      }
    }
