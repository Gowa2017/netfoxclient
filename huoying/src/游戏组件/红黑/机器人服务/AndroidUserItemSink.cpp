...
{	
	...
	for(int i = 0;i<3;i++)
	{
		...
		{
			//当开这个区域需要赔付金币
			SCORE lBKScore = m_lAllBet[i] * cbMultiple[multipleIndex] +   (m_lAllBet[cbBetArea]) *  cbMultiple[multipleIndex] ;
			//这是庄家赚的金币
	...
	if (cbBetArea == 1)
		return lBet/(cbMultiple[multipleIndex] - 1);
