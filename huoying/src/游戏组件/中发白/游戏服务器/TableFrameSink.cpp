...
{ 	
	...
	m_hControlInst = LoadLibrary(TEXT("RedNineBattleServerControl.dll"));
	if ( m_hControlInst )
	{
		typedef void * (*CREATE)(); 
...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	_sntprintf(m_szConfigFileName,sizeof(m_szConfigFileName),TEXT("%s\\RedNineBattle.ini"),szPath);
	...
	case GER_USER_LEAVE:		//用户离开
		{
			...
			if (m_wCurrentBanker!=wChairID)
			{
				...
				{
					for ( WORD wAreaIndex = AREA_ZHONG; wAreaIndex <= AREA_TIANGANG; ++wAreaIndex )
					{
	...
	case GAME_SCENE_END:		//结束状态
		{
			...
			for(WORD wUserChairID = 0; wUserChairID < GAME_PLAYER; ++wUserChairID)
			{
				...
				CMD_S_StatusBetInfo Statusbet;
				ZeroMemory(&Statusbet, sizeof(Statusbet));
				...
				for(WORD i = 0; i < GAME_PLAYER; ++i)
				{
					...
					for(BYTE j = 0;j<AREA_MAX;j++)
					{
						...
						{
							Statusbet.lAndroidBet[j] += m_lPlayBet[i][j];
						}
						else
						{
							Statusbet.lPlayerBet[j] += m_lPlayBet[i][j];
							Statusbet.lPlayerAreaBet[j] = m_lPlayBet[i][j];
							lTotleBet += m_lPlayBet[i][j];
						}
					}
					Statusbet.dwGameID = pUserItem->GetGameID();
					Statusbet.lPlayerTotleBet = lTotleBet;
					m_pITableFrame->SendTableData(wUserChairID, SUB_S_BET_INFO, &Statusbet, sizeof(Statusbet));
					m_pITableFrame->SendLookonData(wUserChairID, SUB_S_BET_INFO, &Statusbet, sizeof(Statusbet));
...
{
	//效验参数
	ASSERT((cbBetArea<=AREA_TIANGANG)&&(lBetScore>0L));
	if ((cbBetArea>AREA_TIANGANG)||(lBetScore<=0L)) return false;
...
void CTableFrameSink::DeduceWinner(BYTE* pWinArea)
{
	...
	//int nWinShangMen=m_GameLogic.CompareCard(m_cbTableCardArray[0],2,m_cbTableCardArray[1],2)==1?true:false;
	int nWinShangMen=m_GameLogic.CompareCard(m_cbTableCardArray[0],2,m_cbTableCardArray[1],2);
