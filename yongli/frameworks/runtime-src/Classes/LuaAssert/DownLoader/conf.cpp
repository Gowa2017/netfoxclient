...
conf_loadfile(conf_t *conf, char *file)
{
	...
	while (!feof(fp)) {
		...
		*tmp = '\0';
		...
		/* Save string option */
		strncpy((char*)dst, value, MAX_STRING - 1);
...
conf_init(conf_t *conf)
{
	...
	*conf->no_proxy = 0;
	...
	//保存时间间隔
	conf->save_state_interval = 3;
	conf->connection_timeout = 3;
	conf->reconnect_delay = 1;
	//获取远程文件 失败 重试次数
	conf->retry_times = 3;
	//文件下载线程数量
	...
	conf->interfaces = (if_t *)malloc(sizeof(if_t));
	if (!conf->interfaces)
		return 0;
...
parse_interfaces(conf_t *conf, char *s)
{
	...
	s[strlen(s) + 1] = 0;
	conf->interfaces = iface = (if_t *)malloc(sizeof(if_t));
	if (!conf->interfaces)
		return 0;
