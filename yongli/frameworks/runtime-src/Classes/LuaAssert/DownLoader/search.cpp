	...
	for (int running = 0; left > 0; axel_nanosleep(delay)) {
		for (int i = 0; i < count; i++) {
			...
			case SPEED_PENDING:
			{
				...
				auto th_search_speed = new std::thread(search_speedtest, &results[i]);
				results[i].speed_thread[0] = th_search_speed;
				if (th_search_speed)
				{
					running++;
				}
			...
			if (results[i].speed_thread[0]->joinable())
			{
				results[i].speed_thread[0]->join();
				running--;
				left--;
				switch (results[i].speed) {
				case SPEED_ACTIVE:
				...
				delete results[i].speed_thread[0];
			}
		}
	}
