...
function FriendLayer:onClickAddFriend()
    ...
    if not appdf.getNodeByName(self.m_friendAddLayer,"EditBoxSearchIput") then
        ...
        EditID:setFontName("base/fonts/round_body.ttf")
        EditID:setPlaceholderFontName("base/fonts/round_body.ttf")
...
function FriendLayer:refreshFriendList()    
    ...
    if 0 == self.m_nFriendCount then
        ...
        lay:addChild(sp)
        local tipLab = cc.Label:createWithTTF("您还没有好友!","base/fonts/round_body.ttf",30)
...
function FriendLayer:updateNotifyList()
    ...
    if #notifyTab ==0 then
        ...
        local tipLab = cc.Label:createWithTTF("暂无消息通知","base/fonts/round_body.ttf",30)
        ...
        for i=#notifyTab,1,-1 do  
            ...
            if curTab.notifyType == 1 then            
                local tip = cc.Label:createWithTTF(curTab.notify.szNickName.."请求成为您的好友","base/fonts/round_body.ttf",30,cc.size(600,0),cc.TEXT_ALIGNMENT_LEFT)
            ...
            elseif curTab.notifyType  == 2 then
                local tip = cc.Label:createWithTTF(curTab.notify,"base/fonts/round_body.ttf",30,cc.size(600,0),cc.TEXT_ALIGNMENT_LEFT)
                tip:setTextColor(cc.c4b(41,82,146,255))
                tip:setAnchorPoint(cc.p(0,0.5))
                tip:setPosition(cc.p(20,notifyItem:getContentSize().height/2))
                notifyItem:addChild(tip)
            ...
            elseif curTab.notifyType  == 3 then
                ...
                local tip = cc.Label:createWithTTF(str,"base/fonts/round_body.ttf",30,cc.size(600,0),cc.TEXT_ALIGNMENT_LEFT)
                tip:setTextColor(cc.c4b(41,82,146,255))
                tip:setAnchorPoint(cc.p(0,0.5))
                tip:setPosition(cc.p(20,notifyItem:getContentSize().height/2))                
                notifyItem:addChild(tip)
...
function FriendLayer:searchResult(userTab)
    ...
    if #userTab ==0 then
        local tipLab = cc.Label:createWithTTF("找不到该用户!!!","base/fonts/round_body.ttf",30);
        ...
        for i=1,#userTab do   
            ...
            local yPos = 60
            local nickName = ClipText:createClipText(cc.size(150,30), userInfoTab.szNickName, "base/fonts/round_body.ttf", 30)
            ...
            local userid = ClipText:createClipText(cc.size(150,30), "ID:" .. userInfoTab.dwGameID, "base/fonts/round_body.ttf", 30)
